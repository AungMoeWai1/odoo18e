...
export class PreparationDisplay extends Reactive {
    ...
    try {
        const newOrders = await this.orm.call(
        ...
        const oldOrders = this.rawData.orders;

        const hasChanged = this.haveOrdersChanged(oldOrders, newOrders);

        if (hasChanged) {
            console.log("🔄 Data changed, updating display...");
            this.rawData.orders = newOrders;
            this.processOrders();
        } else {
...
haveOrdersChanged(oldOrders, newOrders) {
    if (!Array.isArray(oldOrders) || !Array.isArray(newOrders)) return true;
    ...
    for (const newOrder of newOrders) {
        ...
        const oldTableId = oldOrder.table?.id || null;
        const newTableId = newOrder.table?.id || null;

        if (oldTableId !== newTableId) {
            console.log(`🪑 Table changed in order ${newOrder.id}: ${oldTableId} → ${newTableId}`);
            return true;
        }
    }
